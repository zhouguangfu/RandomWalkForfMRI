...
if __name__ == "__main__":
    ...
    rw_atlas_based_aggrator_result = np.zeros((mask.shape[0], mask.shape[1], mask.shape[2], SUBJECT_NUM,
                                               len(ROI) + 1), dtype=np.float)
    for roi_index in range(len(ROI) + 1):
        if roi_index == len(ROI):
            rw_atlas_based_aggrator_result[..., roi_index] = nib.load(RW_AGGRAGATOR_RESULT_DATA_DIR +
                                                                      RW_PROB_BACKGROUND_RESULT_FILE).get_data()
        else:
            rw_atlas_based_aggrator_result[..., roi_index] = nib.load(RW_AGGRAGATOR_RESULT_DATA_DIR + ROI[roi_index] + '_'
                                                                      +RW_ATLAS_BASED_AGGRATOR_RESULT_FILE).get_data()
