...
def atlas_based_aggragator(subject_index):
    ...
    for i in range(20):
        ...
        single_subject_rw_result_dir = RW_AGGRAGATOR_RESULT_DATA_DIR + 'rw/' + str(atlas_num) + '/'
...
def generate_rw_prob_result(atlas_num):
    ...
    for subject_index in range(image.shape[3]):
        single_subject_dir = RW_AGGRAGATOR_RESULT_DATA_DIR + 'rw/' + str(atlas_num) + '/' + str(subject_index) + '_' + RW_PROB_RESULT_FILE
        sibgle_subject_result[..., subject_index] = nib.load(single_subject_dir).get_data()
    nib.save(nib.Nifti1Image(sibgle_subject_result, affine),   RW_AGGRAGATOR_RESULT_DATA_DIR + 'rw/top_rank_' + str(atlas_num)
