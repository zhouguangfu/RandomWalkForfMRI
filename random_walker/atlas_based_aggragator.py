...
if __name__ == "__main__":
    ...
    for roi_index in range(len(ROI) + 1):
        if roi_index == len(ROI):
            nib.save(nib.Nifti1Image(rw_atlas_based_aggrator_result[..., roi_index], affine), RW_RESULT_DATA_DIR + RW_PROB_BACKGROUND_RESULT_FILE)
        else:
            nib.save(nib.Nifti1Image(rw_atlas_based_aggrator_result[..., roi_index], affine), RW_RESULT_DATA_DIR + ROI[roi_index] + '_' +RW_ATLAS_BASED_AGGRATOR_RESULT_FILE)
