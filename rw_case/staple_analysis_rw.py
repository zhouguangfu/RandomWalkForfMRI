...
def generate_single_subject_nii_volume(subject_index):
    ...
    rw_path = RW_AGGRAGATOR_RESULT_DATA_DIR + 'staple/top_rank/' + str(ATLAS_NUM) + '/'+ str(subject_index) + '/'
    ...
    for top_index in range(ATLAS_NUM):
        left_brain_rois_index = left_brain_indexs[top_index]
        right_brain_rois_index = right_brain_indexs[top_index]
...
def random_walker_staple_analysis(subject_index):
    rw_path = RW_AGGRAGATOR_RESULT_DATA_DIR + 'staple/rw/' + str(ATLAS_NUM) + '/' + str(subject_index) + '/'
    if not os.path.exists(rw_path):
        os.makedirs(rw_path)
...
if __name__ == "__main__":
    ...
    # #For single process
    # for subject_index in range(5, SUBJECTS_SESSION_NUMBERS):
