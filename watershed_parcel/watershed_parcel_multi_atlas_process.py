...
def select_optimal_parcel_min_distance(subject_index, size=None):
    ...
    for atlas_index in range(DEFAULT_TOP_RANK):
        ...
        watershed_results_RW[..., atlas_index] = watershed_volume
        #generate the marker
        #r_OFA
...
def select_optimal_parcel_max_region_mean(subject_index, size=None):
    ...
    for atlas_index in range(DEFAULT_TOP_RANK):
        ...
        markers, seg_input, watershed_volume = watershed(image[..., subject_index], 0, 1.6, None, inverse_transformation)
        watershed_results_RW[..., atlas_index] = watershed_volume
...
def atlas_based_aggragator(subject_index):
    ...
    for roi_index in range(len(ROI)):
        ...
        temp[region_results_RW == (roi_index + 1)] = 1
...
if __name__ == "__main__":
    ...
    subject_sum = 3
    rw_atlas_based_aggrator_result = np.zeros((image.shape[0], image.shape[1], image.shape[2], subject_sum, len(ROI)))
    # for subject_index in range(0, 7): #by sessions
    for subject_index in range(0, subject_sum): #by sessions
    ...
    generate_rw_prob_result(rw_atlas_based_aggrator_result, subject_sum)
